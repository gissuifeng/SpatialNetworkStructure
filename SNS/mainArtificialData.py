...
def readPointDataFile(neiDataFile):
    pointList = []
    with open(neiDataFile, 'r') as fr:#neighbor
        fr.readline()
        for line in fr:
...
if __name__ == "__main__":
    flowDataFile = r"./dataSet/artificialFlowData.csv"
    neiDataFile = r"./dataSet/artifical_neighbor.csv"
    pointList = readPointDataFile(neiDataFile)
    readNeighborFile(pointList,neiDataFile)
    pointFlowMat = readFlowData(flowDataFile)
    edgeMSTList = MST(pointList,pointFlowMat)
